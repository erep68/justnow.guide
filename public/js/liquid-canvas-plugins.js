(function($) {
  ...
  $.registerLiquidCanvasPlugin({
    name: "rightRoundedRect",
    defaultOpts: { radius:35 },
    paint: function(area) {
      var ctx = area.ctx;
      var opts = this.opts;
      ctx.beginPath();
  ...
  $.registerLiquidCanvasPlugin({
    ...
    paint: function(area) {
      var grad = area.ctx.createRadialGradient(700,500,800, 0,0,200);
  ...
  $.registerLiquidCanvasPlugin({
    ...
    defaultOpts: { from: "#fff", to:"#666" },
    paint: function(area) {
      var grad = area.ctx.createLinearGradient(0, area.height, area.width, area.height);
      grad.addColorStop(0, this.opts.from);
      grad.addColorStop(0, this.opts.from);
      grad.addColorStop(1, this.opts.to);
      area.ctx.fillStyle = grad;
