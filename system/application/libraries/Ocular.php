...
class Ocular {
	...
	function render($template_name='')
	{
		...
		if (empty($template_name))
		{
			...
			*/
			$file = getcwd() . "/" . $this->ci->config->item('OCU_view_dir') . "/" . $this->ci->config->item('OCU_layout_folder') . "/" . $cont . ".php";
			
                        
			if (file_exists($file))
			{
				// File exists with controller name - use it.
				$template = $this->ci->config->item('OCU_layout_folder') . '/' . $cont;
			} else 
			{
				// controller template doesn't exist, so use the application default
				$template = $this->ci->config->item('OCU_layout_folder') . "/" . $this->ci->config->item('OCU_default_layout');
			}
		} else
		{
			// A template name was passed in, so try to use that template.
			
			$template = $this->ci->config->item('OCU_layout_folder') . "/" . strtolower($this->ci->config->item('OCU_site_name')) . "/" . $template_name;
			if (!$this->view_exists($template))
				$template = $this->ci->config->item('OCU_layout_folder') . "/" . $template_name;
		}
		...
		if (!empty($this->view_data['page_title'])) 
		{
		 	...
		 	{
		 		$this->view_data['page_title'] .= $this->ci->config->item('OCU_site_name_divider') . get_option('site_name');
			    	
		 	} else 
		 	{
		 		$this->view_data['page_title'] = $this->ci->config->item('OCU_site_name_divider') . $this->view_data['page_title'];
	...
	public function yield($cache_me=false)
	{
		...
		$func = (!empty($this->view_name)) ? $this->view_name : $this->active_function();
 	      	$cont = $this->active_controller(TRUE);
		$view = $cont . '/' . strtolower($this->ci->config->item('OCU_site_name')) . '/' . $func;
	...
	{	
		...
		{	
			...
			$view_name = substr($name, strrpos($name, "/"));
			$path = substr($name, 0, strlen($name) - strlen($view_name));
                        
			$view = (!empty($path)? $path : $this->active_controller(TRUE)) . '/' . strtolower($this->ci->config->item('OCU_site_name')) . "/" . $view_name;
                        
			if (!$this->view_exists($view)) 
			$view = (!empty($path)? $path : $this->active_controller(TRUE)) . "/" . $view_name;			
                        
			$this->fetch_view($view, $cache_me, $this->view_data);
			$return = '';
		}
	  ...
	  if ($this->ci->config->item('OCU_development_mode') === TRUE)
	  {
			...
			foreach ($args as $arg)
			{
				...
				} else
				{
                    ...
                    echo '<link rel="stylesheet" type="text/css" href="/' . $this->ci->config->item('OCU_stylesheet_path') . $arg . '.css" media="' . $media . '" />' . "\n";
				}
			}
	  } else
	  {	
		  ...
		  $tag = trim($tag, ',');
		  echo '<link type="text/css" rel="stylesheet" href="/min/?b=' . trim($this->ci->config->item('OCU_stylesheet_path'), '/') . '&f=' . $tag . '&rev='.DAGENSBEDSTE_REVISION.'" media="' . $media . '" />' . "\n";
		}
	...
	//------------------------------------------------------------------------
	public function unregister_styles() {
		$this->styles = array();
	}
    ...
    * into the view.
    */
   function javascripts($type=NULL)
   {
        $this->external_scripts($type);
        $this->inline_scripts();
   }
   ...
   public function register_script($script='', $inline=false)
   {
      if (!empty($script))
      {
         ...
         {
         	if (is_array($script))
         		$this->external_scripts=array_merge($this->external_scripts, $script);
         	else
   ...
   function external_scripts($type=NULL)
   {
   	$tag = '';
        
         $args = $this->default_javascripts($type);
	  ...
	  if (count($this->external_scripts))
	  {
			...
			//only output unique scripts (if one script is registered twice)
			$args = array_unique($args);
	  }
   
   	// Are we in dev mode or production?
   	if ($this->ci->config->item('OCU_development_mode') == TRUE || $this->ci->ocular->get('disable_minify') == 1)
   	{	
   		//
   		// DEVELOPMENT MODE
   		//
	      ...
	      foreach ($args as $script)
	      {
            ...
            if (substr($script, 0, 7)!='http://') {
                $ins_script = '/'.$this->ci->config->item('OCU_javascript_path') . $script.'.js';
            } else $ins_script = $script;
             echo '<script type="text/javascript" src="' .$ins_script.'" ></script>' . "\n";
      ...
      } else 	// PRODUCTION MODE
      {
      	foreach ($args as $arg)
      	{
      		...
      		// Add our tag to the list.
	    		...
	    		echo '<script type="text/javascript" src="'.$arg.'"></script>' . "\n";
	    	}
      	}
	    	
			$tag = trim($tag, ',');
			echo '<script type="text/javascript" src="/min/?b=' . trim($this->ci->config->item('OCU_javascript_path'), '/') . '&f=' . $tag . '&rev='.DAGENSBEDSTE_REVISION.'"></script>' . "\n";
		}
    }
   ...
   public function inline_scripts()
   {
	...
	private function default_javascripts($type=NULL) {
            if(!empty($type))
                return explode(", ", $this->ci->config->item('OCU_javascript_default_collection_'.$type));
            else
	...
	public function set($title='', $data)
	{
		if (!empty($title))
		{
			$this->view_data[$title] = $data;
			return;
		}
	...
	public function set_message($message='', $type='information', $close_btn=false)
	{
	...
	{
		if (!empty($this->_message['message']))
		{
        ...
        public function get_message($type = NULL)	{
    ...
    public function set_backend_message($message='', $type='information', $close_btn=true)
    {
        if (!empty($message) && is_string($message))
        {
            $this->_backmessage['message'] = $message;
            $this->_backmessage['type'] = $type;
            $this->_backmessage['close_btn'] = $close_btn; 
        }
    }
	...
	public function message()
	{
		if (isset($this->_message) && is_array($this->_message))
		{
			...
			echo '</div>';
                ...
                else if (isset($this->_backmessage) && is_array($this->_backmessage))
                {
                    if ( $this->_backmessage['type'] == "error" )             
                        echo '<div class="alert_error"><p><img src="/public/images/backend/icon_error.png" alt="delete" class="mid_align">';
                    else
                        echo '<div class="alert_info"><p><img src="/public/images/backend/icon_info.png" alt="delete" class="mid_align">';

                    echo $this->_backmessage['message'];
                    echo '</p></div>';
                }
	}
