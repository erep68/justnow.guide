...
{
	function img_thumb($src = '', $width=100, $height=66, $index_page = false,$alt = false)
	{
		if ( ! is_array($src) )
		{
			$src = array('src' => $src);
...
{
	function script($src = '', $index_page = FALSE)
	{
		if ( ! is_array($src) )
		{
			$src = array('src' => $src);
		...
		foreach ($src as $k=>$v)
		{
			...
			if ($k == 'src' AND strpos($v, '://') === FALSE)
			{
				...
				if ($index_page === TRUE)
				{
					$scp .= ' src="'.$CI->config->site_url($v).'"';
				}
				else
				{
					$scp .= ' src="'.$CI->config->slash_item('base_url').$v.'"';
				}
			}
			else
			{
